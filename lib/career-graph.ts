...
export type EvidenceSource={
 id:string; label:string; url:string; sourceClass:'first_party_site'|'first_party_public_repository'|'independent_external';
 publicAccessible:boolean; independent:boolean; description:string;
};
export type Evidence={
 id:string; claim:string; value:string; sourceId:string; scope:string;
 publicAccessible:boolean; independentlyVerified:boolean; verificationNote:string;
};
...
export const evidenceSources:EvidenceSource[]=[
 {id:'source:rancorder-case-54',label:'rancorder.dev / 54-site monitoring Decision Record',url:'/cases/54-site-monitoring',sourceClass:'first_party_site',publicAccessible:true,independent:false,description:'本人運営サイト上の公開Case Record。主張内容は公開されるが第三者による独立検証ではない。'},
 {id:'source:rancorder-case-ai',label:'rancorder.dev / AI production delivery Decision Record',url:'/cases/ai-production-delivery',sourceClass:'first_party_site',publicAccessible:true,independent:false,description:'本人運営サイト上の公開Case Record。'},
 {id:'source:rancorder-case-quality',label:'rancorder.dev / Quality rebuild Decision Record',url:'/cases/1400-line-quality-rebuild',sourceClass:'first_party_site',publicAccessible:true,independent:false,description:'本人運営サイト上の公開Case Record。'},
 {id:'source:rancorder-case-sales',label:'rancorder.dev / Sales support PoC Decision Record',url:'/cases/sales-support-poc-operations',sourceClass:'first_party_site',publicAccessible:true,independent:false,description:'本人運営サイト上の公開Case Record。'},
 {id:'source:capability-portfolio',label:'GitHub / capability-portfolio',url:'https://github.com/rancorder/capability-portfolio',sourceClass:'first_party_public_repository',publicAccessible:true,independent:false,description:'公開GitHub repository。18年の現場改善、Sales Ops、AI/DX PM等の自己記述を確認できる。'},
];

export const evidence:Evidence[]=[
 {id:'evidence:54-sites',claim:'54 external sites operated on one monitoring surface',value:'54 sites',sourceId:'source:rancorder-case-54',scope:'monitoring architecture',publicAccessible:true,independentlyVerified:false,verificationNote:'Public first-party record; no independent external verification attached.'},
 {id:'evidence:11-months',claim:'Continuous operation period',value:'11 months',sourceId:'source:rancorder-case-54',scope:'operational reliability',publicAccessible:true,independentlyVerified:false,verificationNote:'Public first-party record; no independent external verification attached.'},
 {id:'evidence:100k-month',claim:'Recurring processing scale',value:'100K+ / month',sourceId:'source:rancorder-case-54',scope:'automation operations',publicAccessible:true,independentlyVerified:false,verificationNote:'Public first-party record; no independent external verification attached.'},
 {id:'evidence:zero-stops',claim:'Business stops caused by system failure',value:'0 system stops',sourceId:'source:rancorder-case-54',scope:'observed operating period',publicAccessible:true,independentlyVerified:false,verificationNote:'Public first-party record; scoped to the stated operating period.'},
 {id:'evidence:ai-production',claim:'Whisper/BERT-family functions connected from PoC to production delivery',value:'2 model families',sourceId:'source:rancorder-case-ai',scope:'AI production delivery',publicAccessible:true,independentlyVerified:false,verificationNote:'Public first-party case statement; customer/project details are not disclosed.'},
 {id:'evidence:quality-tests',claim:'Tests added to previously under-tested legacy logic',value:'30 tests / 1,400 lines',sourceId:'source:rancorder-case-quality',scope:'quality boundary rebuild',publicAccessible:true,independentlyVerified:false,verificationNote:'Public first-party case statement; underlying private code is not published.'},
 {id:'evidence:sales-ops',claim:'Accounts handled by recurring sales-support data operations',value:'38 accounts / 1H cadence',sourceId:'source:rancorder-case-sales',scope:'sales operations PoC',publicAccessible:true,independentlyVerified:false,verificationNote:'Public first-party case statement; client identities are intentionally undisclosed.'},
 {id:'evidence:18-years',claim:'Operational improvement experience described as 18 years',value:'18 years',sourceId:'source:capability-portfolio',scope:'experience origin',publicAccessible:true,independentlyVerified:false,verificationNote:'Public self-authored repository confirms the statement is publicly documented, not independently verified.'},
];

export const expertise:Expertise[]=[
 {id:'expertise:ai-production',slug:'ai-production-delivery',name:'AI Production Delivery',statement:'AIの精度だけでなく、失敗条件・責任境界・Fallback・変更再現性を設計してPoCを本番運用へ移す。',claim:'PoCを「動くデモ」から、失敗時にも運用可能なProductionへ移行できる。',decisionPattern:'Failure condition → Responsibility boundary → Gate → Human fallback → Trace',caseSlugs:['ai-production-delivery'],evidenceIds:['evidence:ai-production','evidence:18-years'],knowledgeQueries:['PoC','Production','AI']},
...
export function sourceForEvidence(id:string){const e=evidence.find(x=>x.id===id);return evidenceSources.find(s=>s.id===e?.sourceId);}
export function evidenceVerificationClass(e:Evidence){
 if(e.independentlyVerified)return 'INDEPENDENTLY VERIFIED';
 if(e.publicAccessible)return 'PUBLIC FIRST-PARTY';
 return 'PRIVATE FIRST-PARTY';
}
