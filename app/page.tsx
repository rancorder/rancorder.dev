...
export default function Home() {
  ...
  return (
    <>
        ...
        <section className="min-h-screen py-16 md:py-24 px-4 md:px-8 bg-black/40 backdrop-blur-sm">
          <div className="max-w-7xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
              {works.map((work, i) => (
                <a key={i} href={work.demo} target="_blank" rel="noopener noreferrer" className="group relative bg-gradient-to-br from-cyan-950/50 to-blue-950/50 border border-cyan-500/30 hover:border-cyan-400/60 transition-all duration-300 overflow-hidden backdrop-blur-sm hover:scale-105 hover:shadow-2xl hover:shadow-cyan-500/30">
        ...
        <section className="min-h-[50vh] py-16 md:py-24 px-4 md:px-8 bg-black/60 backdrop-blur-md">
          <div className="max-w-4xl mx-auto text-center">
            ...
            <div className="space-y-4 md:space-y-6 mb-8 md:mb-12">
              <a href="mailto:hello@rancorder.dev" className="block text-lg md:text-2xl text-cyan-400 hover:text-cyan-300 transition-colors font-mono">
                hello@rancorder.dev
              </a>
      ...
      {contactOpen && (
        <div className="fixed inset-0 z-[100] bg-black/80 backdrop-blur-md flex items-center justify-center p-4 md:p-8" onClick={() => setContactOpen(false)}>
          <div className="bg-gradient-to-br from-cyan-950/90 to-blue-950/90 border-2 border-cyan-500/50 p-6 md:p-10 max-w-2xl w-full relative shadow-2xl shadow-cyan-500/20" onClick={(e) => e.stopPropagation()}>
            <button onClick={() => setContactOpen(false)} className="absolute top-4 right-4 text-cyan-400 hover:text-white text-2xl md:text-3xl font-mono transition-colors">
              ×
            </button>
            ...
            <form className="space-y-4 md:space-y-6">
              <div>
                ...
                <input type="text" className="w-full bg-black/50 border border-cyan-500/30 px-4 py-2 md:py-3 text-white font-mono focus:outline-none focus:border-cyan-400 transition-colors text-sm md:text-base" required />
              </div>
              <div>
                <label className="block text-cyan-300 mb-2 font-mono text-sm md:text-base">メール</label>
                <input type="email" className="w-full bg-black/50 border border-cyan-500/30 px-4 py-2 md:py-3 text-white font-mono focus:outline-none focus:border-cyan-400 transition-colors text-sm md:text-base" required />
              </div>
              <div>
                <label className="block text-cyan-300 mb-2 font-mono text-sm md:text-base">メッセージ</label>
                <textarea rows={5} className="w-full bg-black/50 border border-cyan-500/30 px-4 py-2 md:py-3 text-white font-mono focus:outline-none focus:border-cyan-400 transition-colors resize-none text-sm md:text-base" required />
              </div>
              <button type="submit" className="w-full bg-cyan-500 hover:bg-cyan-400 text-black font-mono font-bold tracking-wider py-3 md:py-4 transition-colors text-sm md:text-base">
                送信
              </button>
            </form>
          </div>
