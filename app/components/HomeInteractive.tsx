...
/**
 * ISO 日付文字列を "YYYY.MM.DD" 形式にフォーマット
 */
...
export default function HomeInteractive({ latestPosts }: HomeInteractiveProps) {
    ...
    <>
      ...
      <header className="fixed top-0 left-0 right-0 z-40 bg-black/40 backdrop-blur-md border-b border-cyan-500/20">
        <div className="max-w-7xl mx-auto px-4 md:px-8 py-4 flex items-center justify-between">
          <a
            href="/"
            className="text-xl md:text-2xl font-bold text-white font-mono hover:text-cyan-300 transition-colors"
          >
            rancorder
          </a>
      ...
      <main className="relative z-10">
        ...
        {/* BLOG */}
        <section className="py-16 md:py-24 px-4 md:px-8 bg-gradient-to-b from-black/40 via-black/60 to-black/60 backdrop-blur-sm">
          <div className="max-w-7xl mx-auto">
            <h2 className="text-3xl md:text-5xl font-bold text-white mb-8 md:mb-16 text-center font-mono tracking-wider">
              BLOG
            </h2>
            ...
            {latestPosts.length > 0 ? (
              <div className="space-y-4 md:space-y-5 mb-12">
                {latestPosts.map((post) => (
                  <a
                    key={post.slug}
                    href={`/blog/${post.slug}`}
                    className="group flex flex-col md:flex-row md:items-center gap-3 md:gap-6 p-5 md:p-6 bg-gradient-to-r from-cyan-950/40 to-blue-950/40 border border-cyan-500/20 hover:border-cyan-400/50 backdrop-blur-sm transition-all duration-300 hover:bg-cyan-950/50"
                  >
                    {/* 日付・カテゴリ */}
                    <div className="flex items-center gap-3 md:flex-col md:items-start md:gap-1 shrink-0 md:w-36">
                      <span className="text-xs text-cyan-400/70 font-mono tracking-wider">
                        {formatDate(post.date)}
                      </span>
                      {post.category && (
                        <span className="text-xs px-2 py-0.5 border border-cyan-500/30 text-cyan-400/80 font-mono tracking-wide">
                          {post.category}
                        </span>
                      )}
                    </div>
                    ...
                    <div className="flex-1 min-w-0">
                      <h3 className="text-base md:text-lg font-bold text-white group-hover:text-cyan-300 transition-colors font-mono leading-snug mb-1">
                        {post.title}
                      </h3>
                      {post.excerpt && (
                        <p className="text-sm text-cyan-100/50 leading-relaxed line-clamp-2">
                          {post.excerpt}
                        </p>
                      )}
                    </div>
                    ...
                    {/* 矢印 */}
                    <div className="hidden md:flex items-center text-cyan-400/40 group-hover:text-cyan-400 transition-colors shrink-0">
                      <span className="font-mono text-lg group-hover:translate-x-1 transition-transform inline-block">
                        →
                      </span>
                    </div>
                  </a>
              ...
              <p className="text-center text-cyan-100/40 font-mono mb-12">
                記事がありません
              </p>
            )}
        ...
        >
          ...
          >
            ...
            <form className="space-y-4 md:space-y-6">
              <div>
                <label className="block text-cyan-300 mb-2 font-mono text-sm md:text-base">
                  名前
                </label>
                <input
                  type="text"
