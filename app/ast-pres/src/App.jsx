  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      for (let i=0; i<drops.length; i++) {
        ...
        ctx.fillStyle = drops[i]*18>H*0.6 ? "rgba(59,130,246,0.9)" : "rgba(0,"+(160+Math.floor(Math.random()*60))+",60,"+(0.3+Math.random()*0.3)+")";
...
function Corners({ color, size, t }) {
  const c = color||"#3b82f6"; const sz = size||24; const th = t||2;
  ...
  return (
    <div style={{display:"flex",gap:4,alignItems:"center",flexWrap:"wrap",justifyContent:"center"}}>
      {Array.from({length:total}).map((_,i)=>(
        <div key={i} style={{width:i<cur?18:6,height:3,background:i<cur?"#2563eb":"rgba(37,99,235,.15)",boxShadow:i<cur?"0 0 3px rgba(37,99,235,.4)":"none",transition:"all .3s ease",borderRadius:2}}/>
      ))}
    </div>
  );
}
...
function ChoiceBtn({ label, sub, color, onClick, chosen }) {
  ...
  return (
    <button onClick={onClick} style={{
      ...
      color: chosen ? c : c+"77",
      fontFamily:VB,
      fontSize:"clamp(.95rem,3.2vw,1.1rem)",fontWeight:400,
...
function Shell({ children }) {
  return (
    <div style={{
      ...
      padding:"clamp(1.5rem,5vw,2rem) clamp(1.5rem,6vw,5vw)",
      display:"flex",
  ...
  return (
    <Shell>
      <div style={{textAlign:"center",display:"flex",flexDirection:"column",alignItems:"center",gap:"clamp(1.5rem,5vw,2.5rem)"}}>
        <div style={{fontFamily:V,fontSize:"clamp(.9rem,3.5vw,1.2rem)",color:"rgba(37,99,235,.7)",letterSpacing:".25em",opacity:phase>=1?1:0,transition:"opacity .6s"}}>
          SYSTEM BOOTING...
        </div>
        <div style={{fontFamily:VB,fontSize:"clamp(1.8rem,7vw,4rem)",color:"#f1f5f9",fontWeight:700,letterSpacing:".04em",lineHeight:1.1,opacity:phase>=2?1:0,transition:"opacity .8s",transform:phase>=2?"translateY(0)":"translateY(12px)"}}>
          AST DECISION<br/><span style={{color:"#3b82f6"}}>SUPPORT MODULE</span>
        </div>
        <div style={{fontFamily:VB,fontSize:"clamp(1.1rem,4.5vw,1.5rem)",color:"#94a3b8",lineHeight:1.8,fontWeight:300,opacity:phase>=3?1:0,transition:"opacity .8s"}}>
          今日は説明じゃなく、<br/>ちょっとした<span style={{color:"#f59e0b"}}>"思考ゲーム"</span>です。
        </div>
        {phase>=3 && (
          <div style={{fontFamily:VB,color:"rgba(100,116,139,.5)",fontSize:"clamp(.75rem,3vw,.8rem)",letterSpacing:".06em",display:"flex",alignItems:"center",gap:".75rem",animation:"fadeIn .8s ease"}}>
  ...
  return (
    <Shell>
      <div style={{textAlign:"center",display:"flex",flexDirection:"column",alignItems:"center"}}>
        <div style={{fontFamily:VB,color:"rgba(37,99,235,.5)",fontSize:"clamp(.7rem,2.5vw,.75rem)",letterSpacing:".2em",fontWeight:500,marginBottom:"clamp(.5rem,2vw,1rem)"}}>Q.</div>
        <div style={{fontFamily:VB,fontSize:"clamp(1.8rem,7vw,4.5rem)",color:"#f1f5f9",fontWeight:700,lineHeight:1.2,textShadow:"none"}}>
          今日、このアポを<br/>受けてもらった<br/><span style={{color:"#f59e0b"}}>一番近い理由は？</span>
        </div>
      </div>
    </Shell>
  );
...
function S2_Select({ onSelect, scenario }) {
  const choices = [
    {id:1,label:"[1]",sub:"たまたま時間が空いてた",        color:"#f1f5f9"},
    {id:2,label:"[2]",sub:"断る理由がなかった",             color:"#f1f5f9"},
    {id:3,label:"[3]",sub:"ちょっと引っかかる言葉があった",color:"#f1f5f9"},
    {id:4,label:"[4]",sub:"正直、よく覚えてない",          color:"#f1f5f9"},
    {id:5,label:"[5]",sub:"実は密かに、依頼先を探していた",color:"#f1f5f9"},
  ];
  return (
    <Shell>
      <div style={{fontFamily:VB,fontSize:"clamp(1.1rem,4.5vw,1.8rem)",color:"#e2e8f0",fontWeight:600,marginBottom:"clamp(1rem,4vw,2rem)"}}>直感で一つだけ。</div>
...
function S3_Shift() {
  return (
    <Shell>
      <div style={{textAlign:"center",display:"flex",flexDirection:"column",alignItems:"center"}}>
        <div style={{fontFamily:VB,fontSize:"clamp(1.5rem,6vw,3.5rem)",color:"#f1f5f9",lineHeight:1.4,fontWeight:600}}>
          その選び方、<br/><span style={{color:"#38bdf8"}}>"判断を任せる相手"</span>を<br/>考える時と<br/>ほぼ同じなんですよね。
        </div>
      </div>
    </Shell>
  );
...
function S5_CoreQ({ glitch }) {
  return (
    <Shell>
      <div style={{textAlign:"center",display:"flex",flexDirection:"column",alignItems:"center"}}>
        <div style={{fontFamily:VB,color:"rgba(100,116,139,.4)",fontSize:"clamp(.65rem,2.5vw,.75rem)",letterSpacing:".15em",fontWeight:400,marginBottom:"clamp(1rem,4vw,1.5rem)"}}>── 出力中</div>
        <div style={{fontFamily:VB,fontSize:"clamp(1.7rem,6.5vw,4rem)",lineHeight:1.3,color:"#f1f5f9",fontWeight:700,marginBottom:"clamp(1.5rem,5vw,3rem)"}}>
          <Glitch trigger={glitch}>{"探している時点で、"}</Glitch><br/>
          <Glitch trigger={glitch}>{"もう一つ"}</Glitch><br/>
          <span style={{color:"#3b82f6",textShadow:"none"}}>
...
function S6_Attack({ glitch }) {
  return (
    <Shell>
      <div style={{fontFamily:VB,fontSize:"clamp(1.5rem,6vw,3.2rem)",lineHeight:1.4,color:"#f1f5f9",fontWeight:600}}>
        <Glitch trigger={glitch}>{"それ、"}</Glitch><br/>
        <Glitch trigger={glitch}>{"新しいものへの期待より──"}</Glitch>
        <br/><br/>
        <span style={{color:"#f59e0b",textShadow:"0 0 30px rgba(245,158,11,.5)",display:"block"}}>
          <Glitch trigger={glitch}>{'"前回と同じにしたくない"'}</Glitch>
        </span>
        <span style={{color:"#f1f5f9"}}>という気持ちに近くないですか？</span>
      </div>
    </Shell>
  );
}
...
function CAE3D() {
  ...
  useEffect(() => {
    ...
    // Wireframe overlay
    const wfMat = new THREE.MeshBasicMaterial({ color: 0x3b82f6, wireframe: true, transparent: true, opacity: 0.08 });
  ...
  return (
    <div style={{ position: "relative", width: "100%", marginTop: "1rem" }}>
      ...
      <div style={{
        ...
        {[["高応力","#ff2200"],["中応力","#f59e0b"],["低応力","#38bdf8"]].map(([lbl,clr]) => (
          <div key={lbl} style={{ display: "flex", alignItems: "center", gap: 5 }}>
            <div style={{ width: 10, height: 10, background: clr, flexShrink: 0 }} />
            <span style={{ fontFamily: V, fontSize: "clamp(.6rem,2vw,.7rem)", color: "#94a3b8" }}>{lbl}</span>
          </div>
        ))}
      </div>
      {/* Labels */}
      <div style={{
        ...
        fontFamily: V, fontSize: "clamp(.6rem,2vw,.7rem)", color: "rgba(59,130,246,.4)",
        textAlign: "right", lineHeight: 1.8,
      }}>
        <div>ドラッグで回転</div>
        <div style={{ color: "rgba(59,130,246,.5)" }}>■ 固定端</div>
        <div style={{ color: "rgba(239,68,68,.5)" }}>▼ 荷重点</div>
      </div>
    </div>
  );
}
...
function JigImages() {
  return (
    <div style={{display:"flex",flexDirection:"column",gap:6,marginTop:"1rem"}}>
      <div style={{fontFamily:"'Share Tech Mono',monospace",fontSize:"clamp(.65rem,2.5vw,.75rem)",color:"rgba(59,130,246,.4)",letterSpacing:".15em",marginBottom:2}}>
        WORKS ── 機械設計実績
      </div>
      <img src={"data:image/jpeg;base64,"+JIG_TOP}
        style={{width:"100%",border:"1px solid rgba(59,130,246,.2)",display:"block"}} alt="治具設計例1"/>
      <img src={"data:image/jpeg;base64,"+JIG_BOT}
        style={{width:"100%",border:"1px solid rgba(59,130,246,.2)",display:"block"}} alt="治具設計例2"/>
    </div>
  );
}
...
function LayoutImage() {
  return (
    <div style={{display:"flex",flexDirection:"column",gap:6,marginTop:"1rem"}}>
      <div style={{fontFamily:"'Share Tech Mono',monospace",fontSize:"clamp(.65rem,2.5vw,.75rem)",color:"rgba(245,158,11,.4)",letterSpacing:".15em",marginBottom:2}}>
        WORKS ── レイアウト設計実績
      </div>
      <img src={"data:image/jpeg;base64,"+LAYOUT_IMG}
        style={{width:"100%",border:"1px solid rgba(245,158,11,.2)",display:"block"}} alt="レイアウト設計例"/>
    </div>
  );
}
function S_Interest() {
  ...
  const adv = [
    {icon:"🎯", title:"技術力",             txt:"用途に応じた的確な判断。シンプルかつメンテナンス性に優れた設計。",                       color:"#3b82f6"},
    {icon:"💬", title:"コミュニケーション", txt:"対面・Web会議を駆使し、「求められていること」を言語化してから設計に入る。",                color:"#38bdf8"},
    {icon:"⚡", title:"スピード",           txt:"3D設計による修正極小化。短納期と品質を両立し、トータルコストを下げる。",                  color:"#f59e0b"},
  ];
  ...
  return (
    <Shell>
      <div style={{fontFamily:V,color:"#3b82f6",fontSize:"clamp(.7rem,2.5vw,.8rem)",letterSpacing:".2em",marginBottom:"clamp(.75rem,3vw,1.25rem)"}}>
        SCOPE ── アストができること
      </div>
          ...
          <div key={b.id}>
            ...
            >
              ...
              <div style={{flex:1,minWidth:0}}>
                <div style={{fontFamily:V,fontSize:"clamp(1.1rem,4vw,1.4rem)",color:openBiz===b.id?b.color:"#f1f5f9",lineHeight:1}}>{b.title}</div>
            ...
            {openBiz===b.id && (
              <div style={{
                ...
                <div style={{fontFamily:V,fontSize:"clamp(.9rem,3.5vw,1.1rem)",color:"#94a3b8",lineHeight:1.7,marginBottom:".75rem",marginTop:b.id==="cae"?"1rem":0}}>{b.detail}</div>
      ...
      {/* 強みバー */}
      <div style={{fontFamily:V,color:"rgba(59,130,246,.4)",fontSize:"clamp(.65rem,2.5vw,.75rem)",letterSpacing:".2em",marginBottom:"clamp(.5rem,2vw,.75rem)"}}>
        ADVANTAGE ──
      </div>
      <div style={{display:"flex",flexDirection:"column",gap:"clamp(.35rem,1.5vw,.5rem)"}}>
        {adv.map(a=>(
          ...
          >
            ...
            {openAdv===a.title && (
              <div style={{fontFamily:V,fontSize:"clamp(.85rem,3.5vw,1rem)",color:"#94a3b8",lineHeight:1.7,marginTop:".6rem",paddingTop:".6rem",borderTop:"1px solid "+a.color+"22",animation:"fadeIn .2s ease"}}>
                {a.txt}
              </div>
            )}
          </button>
    ...
    <Shell>
      <div style={{textAlign:"center",display:"flex",flexDirection:"column",alignItems:"center"}}>
        <div style={{fontFamily:V,fontSize:"clamp(1.2rem,5vw,2rem)",color:"#888",marginBottom:"clamp(1.2rem,4vw,2rem)"}}>
          さっき話してくれた<br/>あの<span style={{color:"#f59e0b"}}>"ズレた瞬間"</span>。
        </div>
        <div style={{padding:"clamp(1.5rem,6vw,3rem) clamp(1.5rem,6vw,4rem)",border:"1px solid rgba(37,99,235,.3)",background:"rgba(37,99,235,.04)",position:"relative",width:"100%",maxWidth:600}}>
          <Corners color="#3b82f6" size={24} t={2}/>
          <div style={{position:"absolute",inset:0,background:"linear-gradient(180deg,transparent,rgba(59,130,246,.05),transparent)",pointerEvents:"none"}}/>
          <div style={{fontFamily:V,fontSize:"clamp(1.5rem,6vw,3rem)",color:"#f1f5f9",lineHeight:1.6,textShadow:"none"}}>
            うちは、<br/>その<span style={{color:"#f59e0b",textShadow:"none"}}>"ズレ"</span>を<br/>最初に設計に組み込む役割で<br/><span style={{color:"#3b82f6",textShadow:"none"}}>呼ばれます。</span>
          </div>
        </div>
      </div>
    </Shell>
...
function S12_End() {
  ...
  return (
    <Shell>
      <div style={{textAlign:"center",display:"flex",flexDirection:"column",alignItems:"center",gap:"clamp(1.5rem,5vw,2.5rem)"}}>
        <div style={{fontFamily:VB,fontSize:"clamp(.75rem,3vw,.85rem)",color:"rgba(37,99,235,.45)",letterSpacing:".25em",fontWeight:500,animation:"shimmer 3s ease-in-out infinite"}}>
          SESSION END.
        </div>
        <div style={{width:120,height:1,background:"linear-gradient(90deg,transparent,rgba(37,99,235,.3),transparent)"}}/>
        <div style={{fontFamily:VB,fontSize:"clamp(1.5rem,6vw,3.5rem)",color:"#f1f5f9",lineHeight:1.35,fontWeight:700,opacity:phase?1:0,transition:"opacity 1s",textShadow:"none"}}>
          今日は、<br/><span style={{color:"#3b82f6"}}>何か気付きはありましたか？</span>
        </div>
        <div style={{marginTop:"1rem",fontFamily:VB,color:"rgba(148,163,184,.35)",fontSize:"clamp(.7rem,2.5vw,.75rem)",letterSpacing:".04em",fontWeight:400}}>
          株式会社アスト / AST Co., Ltd.
        </div>
      </div>
    </Shell>
...
export default function App() {
  ...
  const css = `
    @import url('https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700&family=Noto+Sans+JP:wght@300;400;500;700&family=Share+Tech+Mono&display=swap');
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
    html,body,#root{width:100%;height:100%;overflow:hidden;cursor:default;background:#0d1526;-webkit-text-size-adjust:100%}
    body{color:#e2e8f0;font-family:'Inter','Noto Sans JP',sans-serif;touch-action:pan-y;-webkit-font-smoothing:antialiased}
  ...
  return (
    <div style={{width:"100vw",height:"100vh",background:"#0d1526",overflow:"hidden",position:"relative"}}>
      ...
      >
        ...
        <div style={{
          ...
          background:"rgba(13,21,38,.98)",backdropFilter:"blur(8px)",
          fontFamily:V,
          ...
          <div style={{display:"flex",gap:"clamp(.5rem,2vw,1.5rem)",alignItems:"center",minWidth:0}}>
            <span style={{color:"#2563eb",fontSize:"clamp(.8rem,3vw,.9rem)",fontWeight:600,letterSpacing:".05em",flexShrink:0}}>AST</span>
            {!isMobile && <span>DECISION_SUPPORT</span>}
            {scenario && <span style={{color:"#f59e0b",flexShrink:0}}>SC_{scenario}</span>}
            <span style={{color:"#10b981",animation:"pulse 2s ease-in-out infinite",flexShrink:0}}>● LIVE</span>
          </div>
          <MiniDots cur={idx+1} total={TOTAL}/>
          <div style={{fontFamily:V,fontSize:"clamp(.65rem,2.5vw,.75rem)",color:"rgba(59,130,246,.4)",flexShrink:0}}>
            {String(idx+1).padStart(2,"0")}/{String(TOTAL).padStart(2,"0")}
            {!isMobile && <span style={{color:"rgba(148,163,184,.25)",marginLeft:"1rem"}}>▸ {LABELS[idx]}</span>}
          </div>
        </div>
        ...
        <div style={{
          ...
          {!isMobile
            ...
            : <span style={{color:"rgba(100,116,139,.3)"}}>← スワイプ →</span>
          }
          <div style={{display:"flex",gap:"clamp(.75rem,3vw,1.5rem)"}}>
            <button
              onClick={e=>{e.stopPropagation();go(idx-1);}}
              disabled={idx===0}
              style={{background:"none",border:"none",color:idx===0?"#1e293b":"rgba(59,130,246,.45)",cursor:idx===0?"default":"pointer",fontFamily:V,fontSize:"clamp(.8rem,3.5vw,.9rem)",letterSpacing:".08em",padding:"clamp(.4rem,2vw,.5rem) clamp(.5rem,2vw,.75rem)",minWidth:"clamp(44px,12vw,60px)",WebkitTapHighlightColor:"transparent"}}
            >
              ◀ PREV
            </button>
            <button
              ...
              style={{background:"none",border:"1px solid rgba(59,130,246,.2)",color:idx===TOTAL-1?"#1e293b":"rgba(59,130,246,.8)",cursor:idx===TOTAL-1?"default":"pointer",fontFamily:V,fontSize:"clamp(.8rem,3.5vw,.9rem)",letterSpacing:".08em",padding:"clamp(.4rem,2vw,.5rem) clamp(.5rem,2vw,.75rem)",minWidth:"clamp(44px,12vw,60px)",WebkitTapHighlightColor:"transparent"}}
            >
              NEXT ▶
            </button>
          </div>
